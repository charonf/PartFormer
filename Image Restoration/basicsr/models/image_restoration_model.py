...
class ImageRestorationModel(BaseModel):
    ...
    def init_training_settings(self):
        ...
        if train_opt.get('fre_opt'):
            fre_type = train_opt['fre_opt'].pop('type')
            cri_fre_cls = getattr(loss_module, fre_type)
            self.cri_fre = cri_fre_cls(**train_opt['fre_opt']).to(
                self.device)
        else:
            self.cri_fre = None
    ...
    def optimize_parameters(self, current_iter, tb_logger):
        ...
        if self.cri_fre:
            l_fre = 0.
            for pred in preds:
                l_fre += self.cri_fre(pred, self.gt)
