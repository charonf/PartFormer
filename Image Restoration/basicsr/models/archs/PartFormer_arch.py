...
class PFN(nn.Module):
    ...
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        ...
        self.act = act_layer()
        self.conv = Partial_conv3(dim=in_features, n_div=4, forward='split_cat')
...
class Spatial_Attention(nn.Module):
    ...
    def __init__(self, dim, idx, split_size=[8,8], dim_out=None, num_heads=6, attn_drop=0., proj_drop=0., qk_scale=None, position_bias=True):
        ...
        if self.position_bias:
            self.pos = DynamicPosBias(self.dim // 4, self.num_heads, residual=True)
            # generate mother-set
    ...
    def __init__(self, dim, num_heads,
        ...
        self.attns = nn.ModuleList([
                Spatial_Attention(
                    ...
                    qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, position_bias=False)
...
class PSA(nn.Module):
    ...
    def __init__(self, dim, num_heads, n_div,
                 reso=64, split_size=[8,8], shift_size=[1,2], qkv_bias=False, qk_scale=None,
                 drop=0., attn_drop=0., rg_idx=0, b_idx=0):
        super().__init__()
            # DCTB
        self.conv =nn.Conv2d(dim,dim,1)
        self.dim_p = dim // n_div
        self.dim_untouched = dim - self.dim_p
        self.attn = SA(self.dim_p, num_heads=num_heads, reso=reso, split_size=split_size, shift_size=shift_size, qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop, attn_drop=attn_drop, rg_idx=rg_idx, b_idx=b_idx
            )

    def forward(self, x):
        B,C,H,W = x.shape
        x1,x2= torch.split(x,[self.dim_p,self.dim_untouched],dim=1)
        x1 = self.attn(x1,H,W)
        x = torch.cat((x1,x2),1)
        x = self.conv(x)       
        return x
...
class ATB(nn.Module):
    def __init__(self, dim, num_heads,n_div, reso=64, split_size=[2,4],shift_size=[1,2], expansion_factor=4., qkv_bias=False, qk_scale=None, drop=0.,
        ...
        if b_idx % 2 == 0:
            # DSTB
            self.attn = PSA(
                dim, num_heads=num_heads,n_div=n_div, reso=reso, split_size=split_size, shift_size=shift_size, qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop, attn_drop=attn_drop, rg_idx=rg_idx, b_idx=b_idx
            )
        else:
            # DCTB
        ...
        self.ffn = PFN(in_features=dim, hidden_features=ffn_hidden_dim, out_features=dim, act_layer=act_layer)
